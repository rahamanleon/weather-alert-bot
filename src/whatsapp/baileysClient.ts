...
export class WhatsAppClient extends EventEmitter {
  ...
  constructor(private config: WhatsAppClientConfig) {
    ...
    this.credsPath = path.resolve(config.sessionPath, 'creds.json');
  }

  /**
   * Check if valid credentials exist
   */
  private checkCredentials(): { exists: boolean; valid: boolean; registered?: boolean } {
    ...
    try {
      ...
      console.error('❌ Error reading credentials:', (err as Error).message);
   ...
   */
  private getCredentialInfo(): string {
  ...
  async connect(): Promise<boolean> {
    ...
    if (!credCheck.exists) {
      ...
      try {
        ...
        console.log('⚠️  Could not clear credentials:', (err as Error).message);
      }
      this.requiresAuth = true;
    } else {
    ...
    try {
      ...
      return await new Promise((resolve) => {
        ...
        const checkConnection = (update: any) => {
  ...
  async sendMessageImmediate(phoneNumber: string, message: string): Promise<boolean> {
    if (!this.isConnected || !this.socket) {
    ...
    try {
      ...
      console.error(`❌ Failed to send message to ${phoneNumber}:`, (error as Error).message);
      return false;
    }
  }
  ...
  private toWhatsAppJid(recipient: string): string {
   ...
   */
  getQRCode(): { qr?: string; timestamp?: Date; expiresAt?: Date } {
  ...
  getAuthStatus(): {
    ...
    const credCheck = this.checkCredentials();
    let qrExpiresIn: number | undefined;
  ...
  clearCredentials(): boolean {
    try {
      ...
      console.error('❌ Error clearing credentials:', (err as Error).message);
      return false;
    }
  }

...
export class Geocoder {
  ...
  async reverseGeocode(lat: number, lon: number): Promise<GeocodeResult> {
    ...
    try {
      const response = await fetch(
        `${this.nominatimUrl}/reverse?format=json&lat=${lat}&lon=${lon}&zoom=10&addressdetails=1`
      );
  ...
  async geocodeCity(cityName: string, countryCode?: string): Promise<GeocodeResult | null> {
  ...
  formatCoordinate(lat: number, lon: number, format: 'decimal' | 'dms' = 'decimal'): string {
    if (format === 'dms') {
  ...
  preloadCommonCities(): void {
    const commonCities: GeoLocation[] = [
